...
class GeneralProperties(bpy.types.PropertyGroup):
    ...
    number_of_rider_sets = bpy.props.IntProperty(
        name="Rider Sets",
        description=("Number of unqique sets of riders. Usually just the amount of peeps for this vehicle/ride. "
                     "Some rides for example only expect peeps in sets of 2 to lower the amount of required "
                     "graphics. This is often done on vehicles which carry 4 or more riders."),
        default=0,
        min=0)

    number_of_animation_frames = bpy.props.IntProperty(
        name="Animation Frames",
        description=("Number of animation frames. For example in use for swinging, rotating or animated ride "
                     "vehicles, animated rides, and animated scenery"),
        default=1,
        min=1)
